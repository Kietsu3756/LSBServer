...
then
    ...
    sudo apt install git python3 python3-pip g++-9 cmake make libluajit-5.1-dev libzmq3-dev libssl-dev zlib1g-dev mariadb-server libmariadb-dev screen -y || exit
...
if [ ! -e install.sh ]
then
    git clone --recursive https://github.com/Kietsu3756/KietsuFFXIServer.git
    cd KietsuFFXIServer
fi
...
echo -e "\nCreate an admin MySQL user. (default: $FFXIDBUSER)"
...
if [ -z "$XI_PASSWORD" ]
then
    XI_PASSWORD="$FFXIDBPW"
fi
echo "Name your database. (default: $FFXIDB)"
...
# Update server name
sed -i "s/servername: Nameless/servername: Kietsu/g" conf/login.conf

# Update server message
> conf/server_message.conf
echo "Hello! Welcome to Kietsu server, we hope you enjoy your time!" >> conf/server_message.conf
